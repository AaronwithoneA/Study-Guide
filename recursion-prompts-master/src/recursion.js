...
var compareStr = function(str1, str2) {
  ...
  ((str2.length <=2) && !(str1.length <=2))) {
    return false;
  } else if (str1.length <= 2 && str2[1] === str1[1]) {
    return true
  } else {
...
var countOccurrence = function(array, value) {
  ...
  if (array[0] === value && array[array.length -1 ] === value){
    current = 2
  } else if (array[0] === value || array[array.length -1 ] === value){
    current = 1
  }
    return current + countOccurrence(array.slice(1, array.length - 1), value)
...
var countKeysInObj = function(obj, key) {
  ...
  for(let attr in obj) {
    ...
    if (typeof obj[attr] === 'object') {
      sum += countKeysInObj(obj[attr], key);
    }
  }
  return sum;
};
...
var capitalizeFirst = function(array) {
  if (array.length === 1) {
    return [array[0].slice(0,1).toUpperCase() + array[0].slice(1)];
  }
...
var flatten = function(array) {
  ...
  for(let i = 0; i < array.length; i++) {
    ...
    if (Array.isArray(current)) {
      flattened = flattened.concat(flatten(current));
    } else {
...
var compress = function(list) {
  ...
  let next = compress(list.slice(1));
  if (next.indexOf(list[0]) === -1) {
    return compressed.concat(next);
  }
...
var augmentElements = function(array, aug) {
  ...
  let augmentedElement = array[0];
  augmentedElement.push(aug)
  return [augmentedElement].concat(augmentElements(array.slice(1), aug))
};
